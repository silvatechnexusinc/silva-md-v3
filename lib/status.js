...
class StatusHandler {
    /**
     * Handle status & newsletter logic
     * Executed inside the SAME messages.upsert handler
     */
    async handle({ messages, type, sock, config, logMessage, unwrapStatus, saveMedia }) {
        try {
            ...
            // ✅ Auto-follow newsletters (runs safely once per upsert)
            ...
            for (const m of messages) {
                ...
                try {
                    ...
                    const { inner, msgType } = unwrapStatus
                        ? unwrapStatus(m)
                        : { inner: m.message, msgType: Object.keys(m.message || {})[0] };
